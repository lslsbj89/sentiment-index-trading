"""
买入策略对比实验 - Walk-Forward验证版本
使用 fear_greed_index_s3 表 (Smoothing=3)

Walk-Forward: 4年训练 → 1年测试，滚动窗口
对比5种买入策略:
...
def run_walk_forward(symbol):
    ...
    all_portfolios = {name: [] for name in strategies.keys()}
    ...
    for window in WINDOWS:
        ...
        final_value, trades, portfolio_df, cash, position, entry_price = run_strategy_a(
            test_df, test_buy_t, and_t, or_t, state['cash'], state['position'], state['entry_price']
        )
        ...
        print(f"    Test:  Buy<{test_buy_t:.0f} → {test_ret:+.1f}% | Trades: {len([t for t in trades if t['type']=='BUY'])}")
        ...
        if not portfolio_df.empty:
            all_portfolios['A.Baseline'].append(portfolio_df)
        ...
        print(f"    Test:  Buy<{test_buy_t:.0f} & P>MA10 → {test_ret:+.1f}% | Trades: {len([t for t in trades if t['type']=='BUY'])}")
        ...
        if not portfolio_df.empty:
            all_portfolios['B.MA_Confirm'].append(portfolio_df)
        ...
        print(f"    Test:  Buy<{test_buy_t:.0f} (40%+40%) → {test_ret:+.1f}% | Trades: {len([t for t in trades if t['type']=='BUY'])}")
        ...
        if not portfolio_df.empty:
            all_portfolios['C.Time_Staged'].append(portfolio_df)
        ...
        final_value, trades, portfolio_df, cash, position, entry_price, bought_levels = run_strategy_d(
            test_df, and_t, or_t, state['cash'], state['position'], state['entry_price'],
        ...
        print(f"    Test:  4-level staged → {test_ret:+.1f}% | Trades: {len([t for t in trades if t['type']=='BUY'])}")
        ...
        if not portfolio_df.empty:
            all_portfolios['D.Threshold_Staged'].append(portfolio_df)
        ...
        print(f"    Test:  Buy<{test_buy_t:.0f} (reversal) → {test_ret:+.1f}% | Trades: {len([t for t in trades if t['type']=='BUY'])}")
        ...
        if not portfolio_df.empty:
            all_portfolios['E.Reversal_Confirm'].append(portfolio_df)

    return strategies, all_results, all_trades, all_portfolios


def calculate_final_metrics(strategies, all_trades):
    """计算最终指标"""
    metrics = {}

    for name, state in strategies.items():
        final_value = state['cash'] + state['position'] * 100  # 假设最终价格
        # 实际final_value从结果中获取
...
def main():
    ...
    print("="*70)
    print("买入策略对比实验 - Walk-Forward验证")
    print(f"股票: {TEST_SYMBOL}")
    print("Walk-Forward: 4年训练 → 1年测试")
    print("="*70)

    # 运行Walk-Forward
    strategies, all_results, all_trades, all_portfolios = run_walk_forward(TEST_SYMBOL)

    # 打印最终结果
    print("\n" + "="*70)
    print("Walk-Forward 最终结果对比")
    print("="*70)
    ...
    summary_df.to_csv(f'wf_strategy_comparison_{TEST_SYMBOL}_{timestamp}.csv', index=False)
    ...
    if all_trades_list:
        ...
        trades_df.to_csv(f'wf_trades_all_{TEST_SYMBOL}_{timestamp}.csv', index=False)

    print(f"\n输出文件:")
    print(f"  - wf_strategy_comparison_{TEST_SYMBOL}_{timestamp}.csv")
    print(f"  - wf_trades_all_{TEST_SYMBOL}_{timestamp}.csv")
    ...
    print(f"\n  Walk-Forward验证最佳策略: {best_strategy[0]}")
