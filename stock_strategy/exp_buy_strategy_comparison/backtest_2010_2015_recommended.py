...
def load_price(symbol):
    ...
    price_df = loader.load_ohlcv(symbol, start_date="2009-01-01")
    loader.close()
    return price_df
...
def run_backtest(df, and_threshold, or_threshold):
    ...
    # 计算买入持有收益
    buy_hold_return = (df['Close'].iloc[-1] / df['Close'].iloc[0] - 1) * 100 if len(df) > 0 else 0

    # 计算年化收益
    years = (df.index[-1] - df.index[0]).days / 365.25 if len(df) > 0 else 1
...
def main():
    print("=" * 100)
    print("  2010-2015年度回测 - 使用2026推荐参数 (早期样本外测试)")
    print("=" * 100)
    print(f"\n  回测期间: {TEST_START} ~ {TEST_END} (6年)")
    print(f"  测试目的: 验证推荐参数在更早期历史数据上的适用性")
    print(f"  市场背景: 2010-2015 后金融危机复苏期")
    ...
    for symbol in RECOMMENDED_PARAMS.keys():
        ...
        try:
            ...
            if len(test_df) < 100:
                print(f"  ⚠️ 数据不足 ({len(test_df)}天)，跳过")
                skipped.append(symbol)
                continue

            # 检查数据起始日期
            data_start = test_df.index[0].strftime('%Y-%m-%d')
    ...
    # 汇总表
    print(f"\n\n{'═'*100}")
    print(f"  2010-2015 回测汇总 (早期样本外测试)")
    print(f"{'═'*100}")

    if skipped:
        print(f"\n  ⚠️ 跳过的股票 (数据不足): {', '.join(skipped)}")

    print(f"\n  {'股票':<8} {'数据起始':<12} {'总收益':>12} {'年化':>10} {'买入持有':>12} {'超额':>12} {'最大回撤':>10} {'交易':>6}")
    print(f"  {'-'*95}")
    ...
    for r in results:
        ...
        print(f"  {r['symbol']:<8} {r['data_start']:<12} {r['total_return']:>+11.1f}% {r['annualized_return']:>+9.1f}% {r['buy_hold_return']:>+11.1f}% {excess:>+11.1f}% {excess_marker} {r['max_drawdown']:>9.1f}% {r['num_trades']:>6}")
    ...
    print(f"  {'平均':<8} {'':<12} {avg_strategy:>+11.1f}% {'':<10} {avg_buyhold:>+11.1f}% {avg_excess:>+11.1f}%")
    ...
    for symbol, weight in allocations.items():
        ...
        if r:
            ...
            total_weight += weight
            print(f"  {symbol:<8} {weight*100:>7.0f}% {r['total_return']:>+13.1f}% {contrib:>+11.1f}% {'有数据':>10}")
        else:
            print(f"  {symbol:<8} {weight*100:>7.0f}% {'N/A':>14} {'N/A':>12} {'无数据':>10}")

    if total_weight > 0:
        print(f"  {'-'*60}")
        print(f"  {'组合总计':<8} {total_weight*100:>7.0f}% {portfolio_return:>+13.1f}%")
        print(f"  {'买入持有':<8} {'':<8} {buyhold_return:>+13.1f}%")
        print(f"  {'超额收益':<8} {'':<8} {portfolio_return - buyhold_return:>+13.1f}%")

        # 年化计算
        portfolio_annualized = ((1 + portfolio_return/100) ** (1/6) - 1) * 100
        buyhold_annualized = ((1 + buyhold_return/100) ** (1/6) - 1) * 100
    ...
    print(f"""
  测试目的: 验证2026推荐参数在2010-2015数据上的表现
  市场背景: 后金融危机复苏期，整体上涨趋势
...
""")
    ...
    if total_weight > 0:
        print(f"  - 组合超额收益: {portfolio_return - buyhold_return:+.1f}%")

    print(f"""
  参数适用性评估:
""")

    if win_count >= len(results) * 0.7:
        print("  ✓ 推荐参数在早期数据上表现良好，具有较强的泛化能力")
    elif win_count >= len(results) * 0.5:
        print("  ⚠ 推荐参数在早期数据上表现一般，部分股票适用")
    else:
        print("  ✗ 推荐参数在早期数据上表现较差，策略更适合近期市场环境")
