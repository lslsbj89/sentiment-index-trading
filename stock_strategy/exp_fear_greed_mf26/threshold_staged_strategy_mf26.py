"""
阈值分批买入策略 - MoneyFlow=26 版本
=====================================
策略: D.阈值分批 + 乘法放宽
数据源: fear_greed_index (MoneyFlow=26, Smoothing=3)

与生产版本的唯一区别:
- 情绪数据表: fear_greed_index (MoneyFlow周期从13改为26个交易日)
...
def load_sentiment(symbol):
    """从 fear_greed_index 表加载情绪数据 (MoneyFlow=26)"""
...
def run_walk_forward(symbol):
    ...
    print(f"\n{'='*70}")
    print(f"  {symbol} - 阈值分批策略 Walk-Forward 分析 (MoneyFlow=26)")
    print(f"{'='*70}")
    ...
    print(f"  数据源: fear_greed_index (MoneyFlow=26)")
...
def main():
    ...
    print("="*70)
    print("阈值分批买入策略 - Walk-Forward 验证 (MoneyFlow=26)")
    print("="*70)
    print(f"数据源: fear_greed_index (MoneyFlow=26, Smoothing=3)")
    ...
    # ========== 打印汇总 ==========
    print("\n" + "="*70)
    print("最终排名 (2020-2025 连续回测, MoneyFlow=26)")
    print("="*70)
    ...
    if recommendations:
        print("\n" + "="*70)
        print("2026年推荐参数 (基于2021-2024训练, MoneyFlow=26)")
        print("="*70)
    ...
    summary_df.to_csv(f'threshold_staged_mf26_summary_{timestamp}.csv', index=False)

    # 推荐参数CSV
    if recommendations:
        rec_df.to_csv(f'threshold_staged_mf26_params_2026_{timestamp}.csv', index=False)

    # 交易记录CSV
    if all_trades:
        trades_df = pd.DataFrame(all_trades)
        trades_df['date'] = trades_df['date'].dt.strftime('%Y-%m-%d')
        trades_df.to_csv(f'threshold_staged_mf26_trades_{timestamp}.csv', index=False)

    print(f"\n输出文件:")
    print(f"  - threshold_staged_mf26_summary_{timestamp}.csv")
    if recommendations:
        print(f"  - threshold_staged_mf26_params_2026_{timestamp}.csv")
    if all_trades:
        print(f"  - threshold_staged_mf26_trades_{timestamp}.csv")
