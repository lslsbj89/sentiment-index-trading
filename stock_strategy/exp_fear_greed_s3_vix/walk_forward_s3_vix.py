"""
Walk-Forward 分析 - 使用 fear_greed_index_s3_vix 表
四因子模型: 原三因子 + VIX
平滑参数: Smoothing=3
...
def load_sentiment_s3_vix(symbol):
    """从 fear_greed_index_s3_vix 表加载情绪数据 (四因子+VIX, S=3)"""
    conn = psycopg2.connect(**db_config)
    query = """
        SELECT date, smoothed_index
        FROM fear_greed_index_s3_vix
        WHERE symbol = %s
        ORDER BY date
    """
...
def analyze_symbol(symbol):
    """分析单个股票"""
    print(f"\n{'='*70}")
    print(f"  {symbol} Walk-Forward 分析 (S3+VIX 四因子)")
    print(f"{'='*70}")

    # 加载数据
    price_df = load_price(symbol)
    sentiment_df = load_sentiment_s3_vix(symbol)
...
def main():
    ...
    print("="*70)
    print("Walk-Forward 分析 - fear_greed_index_s3_vix (四因子+VIX, S=3)")
    print("="*70)
    ...
    summary_df.to_csv(f'summary_s3_vix_{timestamp}.csv', index=False)

    # 交易记录
    if all_trades:
        trades_df = pd.DataFrame(all_trades)
        trades_df['date'] = trades_df['date'].dt.strftime('%Y-%m-%d')
        trades_df.to_csv(f'trades_s3_vix_{timestamp}.csv', index=False)

    # 2026推荐参数
    rec_df = pd.DataFrame(recommendations)
    rec_df = rec_df.sort_values('total_return', ascending=False)
    rec_df.to_csv(f'recommendations_2026_s3_vix_{timestamp}.csv', index=False)

    # 打印2026推荐参数
    print("\n" + "="*70)
    print("2026年推荐参数 (S3+VIX 四因子, 基于2021-2024训练)")
    print("="*70)
    ...
    # 最终排名
    print("\n" + "="*70)
    print("最终排名 (连续回测 2020-2025, S3+VIX)")
    print("="*70)
    ...
    print(f"\n输出文件:")
    print(f"  - summary_s3_vix_{timestamp}.csv")
    print(f"  - trades_s3_vix_{timestamp}.csv")
    print(f"  - recommendations_2026_s3_vix_{timestamp}.csv")
