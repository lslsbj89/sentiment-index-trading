"""
Walk-Forward 分析 - 使用 fear_greed_index_s5 表 (Smoothing=5)
...
def load_sentiment_s3(symbol):
    """从 fear_greed_index_s5 表加载情绪数据"""
    conn = psycopg2.connect(**db_config)
    query = """
        SELECT date, smoothed_index
        FROM fear_greed_index_s5
        WHERE symbol = %s
        ORDER BY date
    """
...
def analyze_symbol(symbol):
    """分析单个股票"""
    print(f"\n{'='*70}")
    print(f"  {symbol} Walk-Forward 分析 (Smoothing=5)")
    print(f"{'='*70}")
    ...
    return {
        ...
        'results': results,
        'trades': all_trades,
        'recommended_2026': results[-1] if results else None  # W2025的参数用于2026
    }
...
def main():
    ...
    print("="*70)
    print("Walk-Forward 分析 - fear_greed_index_s5 (Smoothing=5)")
    print("="*70)
    ...
    summary_df.to_csv(f'summary_{timestamp}.csv', index=False)
    ...
    rec_df.to_csv(f'recommendations_2026_{timestamp}.csv', index=False)
    ...
    print(f"  - recommendations_2026_{timestamp}.csv")
