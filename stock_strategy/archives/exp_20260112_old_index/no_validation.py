...
def analyze_period(stock_data, sentiment_data, period):
    ...
    print(f"{'='*70}")
    print(f"  训练期: {train_start} ~ {train_end} (4年)")
    ...
    print(f"  最优参数: buy<{best['buy_threshold']}, sell>{best['sell_threshold']}")
    print(f"  训练收益: {best['total_return']:.2%}, 夏普: {best['sharpe_ratio']:.2f}")

    # 直接用于测试期
    print(f"\n[测试期] 直接评估...")
    portfolio, metrics, trades = run_backtest(
        stock_data, sentiment_data,
        best['buy_threshold'], best['sell_threshold'],
        test_start, test_end
    )
    ...
    print(f"  测试收益: {test_return:.2%}")
    print(f"  测试夏普: {test_sharpe:.2f}")
    print(f"  测试回撤: {test_dd:.2%}")
    ...
    if trades:
        ...
        for t in trades:
            ...
            print(f"    买入: {entry} @${t['entry_price']:.2f} → 卖出: {exit_d} @${t.get('exit_price', 0):.2f} | {reason}")
...
def main():
    ...
    print(f"\n{'周期':<18} {'阈值':<12} {'训练收益':>10} {'测试收益':>10} {'夏普':>8} {'回撤':>10} {'交易':>6}")
    print("-" * 80)
    for r in results:
        threshold = f"<{r['buy_threshold']},>{r['sell_threshold']}"
        print(f"{r['label']:<18} {threshold:<12} {r['train_return']:>10.2%} {r['test_return']:>10.2%} "
