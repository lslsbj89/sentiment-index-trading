...
# 最优配置
OPTIMAL_CONFIG = {
    'NVDA': {'buy_th': -10, 'sell_mode': 'and_ma', 'sell_th': 10, 'position_mode': 'dynamic'},
    'TSLA': {'buy_th': -10, 'sell_mode': 'and_ma', 'sell_th': 10, 'position_mode': 'dynamic'},
    'GOOGL': {'buy_th': -5, 'sell_mode': 'threshold', 'sell_th': 30, 'position_mode': 'fixed'},
    'META': {'buy_th': -10, 'sell_mode': 'and_ma', 'sell_th': 5, 'position_mode': 'dynamic'},
    'MSFT': {'buy_th': -10, 'sell_mode': 'threshold', 'sell_th': 30, 'position_mode': 'fixed'},
    'AAPL': {'buy_th': -10, 'sell_mode': 'threshold', 'sell_th': 30, 'position_mode': 'fixed'},
    'AMZN': {'buy_th': -10, 'sell_mode': 'and_ma', 'sell_th': 5, 'position_mode': 'fixed'},
}
...
def run_continuous_backtest(stock_data, sentiment_data, symbol):
    ...
    if len(sell_trades) > 0:
        wins = sum(1 for t in sell_trades if t['trade_return'] > 0)
...
def plot_trades(symbol, result, output_dir):
    ...
    # 标记交易点的组合价值
    for t in trades:
        if t['portfolio_value']:
            marker = '^' if t['type'] == 'BUY' else 'v'
            color = 'green' if t['type'] == 'BUY' else 'red'
            ax2.scatter(t['date'], t['portfolio_value'], color=color, marker=marker, s=80, zorder=5)
    ...
    # 保存
    output_path = os.path.join(output_dir, f'{symbol}_continuous_trades.png')
...
def main():
    print("=" * 80)
    print("连续5年回测 - 动态复利 - 保存交易记录并生成可视化图")
    print("=" * 80)

    # 创建输出目录
    output_dir = 'continuous_backtest_results'
    ...
    for symbol in OPTIMAL_CONFIG.keys():
        ...
        result = run_continuous_backtest(stock_data, sentiment_data, symbol)
        if result:
    ...
    # 打印汇总表
    print("\n" + "=" * 100)
    print("连续5年回测汇总 (2021-2025)")
    print("=" * 100)

    print(f"\n{'股票':<8} {'卖出条件':<12} {'仓位':<8} {'5年收益':>12} {'年化':>10} {'回撤':>10} {'夏普':>8} {'交易':>6} {'胜率':>8}")
    print("-" * 95)
    ...
    for r in sorted(all_results, key=lambda x: x['total_return'], reverse=True):
        ...
        pos_short = '动态' if config['position_mode'] == 'dynamic' else '固定'
        sell_trades = [t for t in r['trades'] if t['type'] == 'SELL']

        print(f"{r['symbol']:<8} {sell_short:<12} {pos_short:<8} {r['total_return']:>12.2%} "
    ...
    avg_annual = np.mean([r['annualized'] for r in all_results])
    ...
    print(f"{'平均':<8} {'':<12} {'':<8} {avg_return:>12.2%} {avg_annual:>10.2%} "
