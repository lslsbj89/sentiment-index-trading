...
def run_backtest_dynamic(stock_data, sentiment_data, year):
    """动态复利仓位回测"""
    ...
    for i, (date, row) in enumerate(price_data.iterrows()):
        ...
        # 无仓位时的买入逻辑 - 动态复利
            ...
            # 关键区别：使用总资产而非现金
            current_total = cash + shares * price  # 此时shares=0，所以就是cash
            position_value = current_total * POSITION_PCT  # 总资产 × 80%
...
def main():
    ...
    for symbol in MAG7_SYMBOLS:
        ...
        for year in TEST_YEARS:
            r = run_backtest_dynamic(stock_data, sentiment_data, year)
            if r:
                results.append(r)
    ...
    for symbol, results in all_results.items():
        ...
        print(f"股票: {symbol}")
        print(f"{'='*90}")
        print(f"\n  {'年份':<6} {'收益率':>10} {'最大回撤':>10} {'夏普率':>10} {'交易':>6} {'止损':>6}")
        print(f"  {'-'*55}")

        for r in results:
            print(f"  {r['year']:<6} {r['return']:>10.2%} {r['max_drawdown']:>10.2%} "
                  f"{r['sharpe']:>10.2f} {r['trades']:>6} {r['and_stops']:>6}")
        ...
        print(f"  {'-'*55}")
        print(f"  {'平均':<6} {avg_ret:>10.2%} {avg_dd:>10.2%} {avg_sharpe:>10.2f}")
    ...
    print(f"\n{'股票':<8} {'平均收益':>10} {'平均回撤':>10} {'平均夏普':>10} {'5年累计':>12} {'年化':>10}")
    print("-" * 70)
    ...
    for s in sorted(summary, key=lambda x: x['cumulative'], reverse=True):
        print(f"{s['symbol']:<8} {s['avg_return']:>10.2%} {s['avg_drawdown']:>10.2%} "
              f"{s['avg_sharpe']:>10.2f} {s['cumulative']:>12.2%} {s['annualized']:>10.2%}")
    ...
    # 与固定仓位对比
    print("\n" + "=" * 90)
    print("动态复利 vs 固定仓位 对比")
    print("=" * 90)

    # 固定仓位结果 (从实验11)
    fixed_results = {
        'NVDA': {'cum': 601.97, 'sharpe': 1.32},
        'TSLA': {'cum': 329.47, 'sharpe': 0.94},
        'AAPL': {'cum': 125.20, 'sharpe': 1.06},
        'GOOGL': {'cum': 120.30, 'sharpe': 0.80},
        'MSFT': {'cum': 94.16, 'sharpe': 0.89},
        'META': {'cum': 89.34, 'sharpe': 0.80},
        'AMZN': {'cum': 45.69, 'sharpe': 0.57}
    }

    print(f"\n{'股票':<8} {'固定仓位':>12} {'动态复利':>12} {'差异':>10} {'更优':>10}")
    print("-" * 60)

    for s in summary:
        symbol = s['symbol']
        fixed = fixed_results.get(symbol, {}).get('cum', 0)
        dynamic = s['cumulative'] * 100
        diff = dynamic - fixed
        better = "动态复利" if diff > 0 else "固定仓位"
        print(f"{symbol:<8} {fixed:>12.0f}% {dynamic:>12.0f}% {diff:>+10.0f}% {better:>10}")
