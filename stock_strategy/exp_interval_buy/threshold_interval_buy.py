...
买入逻辑 (训练期, 不变):
- 原版阈值分批: sentiment < 5/0/-5/-10, 每档25%

买入逻辑 (测试期, 新):
- 当 sentiment < 买入阈值(5) → 触发买入模式, 首次买入20%
- 每隔7天 → 再买入20% (在买入模式中)
- 当 sentiment 重新 > 买入阈值(5) → 一次性买入剩余全部仓位, 退出买入模式
...
# 测试期: 间隔分批参数
INTERVAL_BUY_THRESHOLD = 5      # 买入触发阈值
...
def run_interval_buy(df, and_threshold, or_threshold,
    ...
    买入逻辑:
    - sentiment < INTERVAL_BUY_THRESHOLD → 进入买入模式, 首次买入20%
    - 每隔 INTERVAL_DAYS 天 → 买入20%
    - sentiment > INTERVAL_BUY_THRESHOLD (买入模式中) → 买入剩余全部, 退出买入模式
        ...
        if position == 0 or in_buy_mode:
            ...
            if in_buy_mode:
                ...
                if last_buy_date is None:
                    # 首次买入
                    should_buy = True
                    buy_reason = f"Interval B1: sentiment {current_sentiment:.1f} < {INTERVAL_BUY_THRESHOLD} (enter buy mode)"
                elif current_sentiment >= INTERVAL_BUY_THRESHOLD:
                    # 情绪恢复: 买入剩余全部
                    should_buy = True
                    buy_all_remaining = True
                    buy_reason = f"Recovery ALL: sentiment {current_sentiment:.1f} >= {INTERVAL_BUY_THRESHOLD} (buy remaining)"
                elif (current_date - last_buy_date).days >= INTERVAL_DAYS:
                    # 间隔买入
                    should_buy = True
                    batches_bought += 1
                    buy_reason = f"Interval B{batches_bought+1}: {(current_date - last_buy_date).days}d since last buy"
                ...
                if should_buy:
                    ...
                    if buy_all_remaining:
                        ...
                        # 买入20%
                        target_value = buy_base_capital * INTERVAL_BATCH_PCT
                        shares = int(target_value / buy_price)
...
def run_walk_forward(symbol):
    ...
    print(f"\n{'='*70}")
    print(f"  {symbol} - 间隔分批买入策略 Walk-Forward 分析")
    print(f"{'='*70}")
    ...
    print(f"  测试期买入: threshold<{INTERVAL_BUY_THRESHOLD}, 间隔{INTERVAL_DAYS}天, 每次{INTERVAL_BATCH_PCT*100:.0f}%")
    ...
    for window in WINDOWS:
        ...
        # 测试期: 间隔分批买入
        (final_value, trades, portfolio_df, cash, position, entry_price,
...
def main():
    ...
    print("="*70)
    print("间隔分批买入策略 - Walk-Forward 验证")
    print("="*70)
    print(f"训练期买入: 阈值分批 {BUY_THRESHOLDS}, 每档{BATCH_PCT*100:.0f}%")
    print(f"测试期买入: threshold<{INTERVAL_BUY_THRESHOLD}, 间隔{INTERVAL_DAYS}天, 每次{INTERVAL_BATCH_PCT*100:.0f}%, 恢复时全买")
    ...
    summary_df.to_csv(f'interval_buy_summary_{timestamp}.csv', index=False)

    if all_trades:
        trades_df = pd.DataFrame(all_trades)
        trades_df['date'] = trades_df['date'].dt.strftime('%Y-%m-%d')
        trades_df.to_csv(f'interval_buy_trades_{timestamp}.csv', index=False)

    print(f"\n输出文件:")
    print(f"  - interval_buy_summary_{timestamp}.csv")
    print(f"  - interval_buy_trades_{timestamp}.csv")
